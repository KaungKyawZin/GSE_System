...
export default function AssignedVehicles({
    ...
    async function fetchAll() {
      try {
        ...
        const [vRes, fRes, gRes, uRes] = await Promise.all([
          ...
          fetch("/api/users/user_by_role.php"),
        ]);
        ...
        const vData =
          vJson && vJson.success && Array.isArray(vJson.data) ? vJson.data : [];
        const fData =
          fJson && fJson.success && Array.isArray(fJson.data) ? fJson.data : [];
        const gData =
          gJson && gJson.success && Array.isArray(gJson.data) ? gJson.data : [];
        const uData =
          uJson && uJson.success && Array.isArray(uJson.data) ? uJson.data : [];
        ...
        const gatesNorm = gData.map((g) => ({
          id: g.gate_id,
          name: g.gate_code || g.gate_id,
          raw: g,
        }));
        const usersNorm = uData.map((u) => ({
          id: u.user_id,
          username: u.username,
  ...
  function toggleCheck(index) {
    setChecklist((prev) =>
      prev.map((it, i) => (i === index ? { ...it, ok: !it.ok } : it)),
    );
  }
  ...
  async function runAiCheck(vehicle) {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/predict.php", {
        method: "POST",
        headers: {
      ...
      setAiDetails(prediction);

      if (prediction.result === "Pass") {
        setAiResult("ok");
      } else if (prediction.result === "Fail") {
        setAiResult("fail");

        await autoReject(
          vehicle,
          `AI Failed | Risk: ${prediction.risk} | ${prediction.recommendation}`,
        );
      }
    } catch (error) {
      console.error("AI Error:", error);

      setApiError && setApiError("AI prediction server error");
    }
  }
  ...
  async function manualReject(vehicle, comment) {
    try {
      const payload = {
        title: `Manual reject: ${vehicle.vehicle_number || vehicle.id}`,
        message: comment || "Rejected by inspector",
        vehicle_id: vehicle.id,
      };
      ...
      setApiMessage &&
        setApiMessage("Vehicle rejected and reported to technicians.");
      setAiResult("fail");
    } catch (e) {
  ...
  async function autoReject(vehicle, reason) {
    ...
    try {
      const payload = {
        title: `Auto-reject: ${vehicle.vehicle_number || vehicle.id}`,
        message: reason,
        vehicle_id: vehicle.id,
      };
      await fetch("/api/notifications/create_notification.php", {
      ...
      setApiMessage &&
        setApiMessage("Vehicle auto-rejected and reported to technicians.");
    } catch (e) {
      setApiError &&
        setApiError("Failed to forward auto-reject report: " + e.message);
    }
  }
  ...
  async function approveAndAssign(driverId) {
    ...
    try {
      ...
      setVehicles((prev) =>
        prev.map((v) =>
          v.id === selected.id
            ? { ...v, assigned_driver_id: driverId, status: "Assigned" }
            : v,
        ),
      );
  ...
  return (
    <div>
      ...
      <p className="muted">
        Tap a vehicle card to inspect. AI check will auto-reject failing
        vehicles and notify technicians.
      </p>
      ...
      {!loading && !error && (
        <div className="dashboard-grid">
          ...
          {vehicles.map((v) => {
            const flight =
              flights.find((f) => f.id === v.assigned_flight) ||
              flights.find((f) => f.flight_number === v.assigned_flight) ||
              {};
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ marginTop: 8 }}>
                      Status:{" "}
                      <span
                        className={`status-badge ${v.status && v.status.toLowerCase()}`}
                      >
                        {v.status || "Unknown"}
                      </span>
                    </div>
            ...
            <div className="modal-scroll-area">
              ...
              >
                ...
                <div>
                  ...
                  >
                    {selected.vehicle_number || `#${selected.id}`}
                  </h2>
                  <div style={{ marginTop: "6px" }}>
                    <span
                      className={`status-pill ${selected.status ? selected.status.toLowerCase().replace(/\s+/g, "-") : "available"}`}
                    >
                      {selected.status}
                    </span>
                  </div>
              ...
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                ...
                <div style={{ flex: "1 1 260px" }}>
                  <h4>AI Detection</h4>
                  <p className="muted">
                    Run AI model to automatically validate vehicle condition.
                    You can also perform a manual review.
                  </p>
                  <div
                    style={{
                      display: "flex",
                  ...
                  {aiResult === "ok" && (
                    <div className="feedback-alert success">
                      AI Passed - Vehicle Approved
                    </div>
                  )}

                  {aiResult === "fail" && (
                    <div className="feedback-alert error">
                      AI Failed - Vehicle Rejected
                      {aiDetails && (
                        <div style={{ marginTop: 10 }}>
                          <p>Risk: {aiDetails.risk}</p>
                          <p>Confidence: {aiDetails.confidence}%</p>
                          <p>Recommendation: {aiDetails.recommendation}</p>
                        </div>
                      )}
                    </div>
                  )}
                  {manualMode && (
                    <div style={{ marginTop: 12 }}>
                      ...
                      <textarea
                        ...
                        placeholder="Enter comment for rejection or note"
                        value={manualComment}
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                        <button
                          ...
                          onClick={() => {
                            setAiResult("ok");
                            setApiMessage &&
                              setApiMessage("Manually approved.");
                          }}
                        >
                          Approve
                        </button>
                        <button
                          ...
                          onClick={() => manualReject(selected, manualComment)}
                        >
                          Reject & Report
                        </button>
                      </div>
                  ...
                  {aiResult === "ok" && (
                    <div style={{ marginTop: 16 }}>
                      ...
                      <select
                        ...
                        defaultValue=""
                        onChange={(e) => approveAndAssign(e.target.value)}
                      >
                      ...
                      {assigning && (
                        <div style={{ marginTop: 8 }}>Assigning…</div>
                      )}
                    </div>
                  )}
                </div>
